"""
Shared SharePoint utilities for Azure Functions
"""
...
def get_sharepoint_context() -> ClientContext:
    ...
    try:
        auth_context = AuthenticationContext(f"https://login.microsoftonline.com/{SHAREPOINT_TENANT_ID}")
        ...
        if auth_context.token:
            ...
            raise Exception("Failed to acquire SharePoint authentication token")
    except Exception as error:
        print(f"SharePoint authentication error: {error}")
        raise error

def get_folder_by_name(folder_name: str) -> Optional[Folder]:
